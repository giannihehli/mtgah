...
images = glob.glob("H:/data/calibration/sony/*.JPG")
...
for iname in images:
    ...
    if board == True:
        ...
        #cv2.waitKey(0)
        print("saving detected image: ", os.path.basename(iname))
        cv2.imwrite("H:/data/calibration/" + camera + "/detected/det_" + os.path.basename(iname), img)
    else:
