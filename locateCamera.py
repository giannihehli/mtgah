...
def find_camera_pose(image_points, object_points, camera_matrix, dist_coeffs):
...
def plot_location(rvec, tvec, object_points):
    # plotCamera() config
    plot_range  = 3 # target volume [-plot_range:plot_range]
    ...
    ax_in.plot(object_points[:,0], object_points[:,1], object_points[:,2], ".")
    
    plt.show()
    cv2.waitKey(0)
#    plt.savefig("calibration/" + camera + "/result.pdf")

def plot_camera_pose(ax, rvec, tvec, object_points, camera_length=0.1):
    # Plot object points
...
#    img_undst = image
    ...
    corners_sort = np.ones((len(ids)*4, 2), dtype=np.float32)
    ...
    pattern = 0.001 * np.array([[12.5, 6.2, 0], [99.5, 8, 0], [98.6, 95, 0], [11.5, 93.2, 0], # top left tag
    ...
    image_points = corners_sort
    object_points = pattern
    ...
    # Find camera pose
    rvec, tvec = find_camera_pose(image_points, object_points, camera_matrix, dist_coeffs)

    # Plot camera pose
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    plot_camera_pose(ax, rvec, tvec, object_points)
    plt.show()
    cv2.waitKey(0)

    # Print camera pose
    plot_location(rvec, tvec, object_points)
