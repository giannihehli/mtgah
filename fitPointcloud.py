...
# Load the .ply file
cloud = PyntCloud.from_file('H:/data/cloudcompare/test/final.ply')
...
for index, point in cloud.points.iterrows():
    ...
    image[y, x] = [point['blue'], point['green'], point['red']]
    indices[y, x] = index
...
cloud.points['x'], cloud.points['y'], cloud.points['z'], _ = np.dot(M, np.array([cloud.points['x'], cloud.points['y'], cloud.points['z'], np.ones(cloud.points.shape[0])]))
...
ax.scatter(cloud.points['x'], cloud.points['y'], cloud.points['z'])
