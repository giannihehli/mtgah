        ...
        for vid_path in glob.glob(data_path + 'camera/' + '*.MP4'):
            ...
            for frame in range(frame_count):
                ...
                if not np.array_equal(ids[0:4], correct_ids):
                    print(f'Wrong ids {ids} detected in frame {frame} - {datetime.now()}')
                    # Append the same parameters to lists as the previous frame
                    points_right = np.append(points_right, points_right[frame-1] + (points_right[frame-1]-points_right[frame-2]))
                    points_left = np.append(points_left, points_left[frame-1] + (points_left[frame-1]-points_left[frame-2]))
                    points_bottom = np.append(points_bottom, points_bottom[frame-1] + (points_bottom[frame-1]-points_bottom[frame-2]))
                    time_frame = np.append(time_frame, time_frame[frame-1])

                    # Calculate velocity with conversion factor to [mm/s]
                    if frame >= 1:
                        velocity_right = np.append(velocity_right, velocity_right[frame-1] + (velocity_right[frame-1]-velocity_right[frame-2]))
                        velocity_left = np.append(velocity_left, velocity_left[frame-1] + (velocity_left[frame-1]-velocity_left[frame-2]))
                        velocity_bottom = np.append(velocity_bottom, velocity_bottom[frame-1] + (velocity_bottom[frame-1]-velocity_bottom[frame-2]))
